-- Migration 004: Fix Household Staples Integration (Version 2)
...
-- Fixed: Removed problematic DROP statements that reference non-existent tables
...
CREATE TABLE IF NOT EXISTS household_staples (
...
CREATE TABLE IF NOT EXISTS staples_usage_history (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_household_staples_household_id ON household_staples(household_id);
CREATE INDEX IF NOT EXISTS idx_household_staples_active ON household_staples(household_id, is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_household_staples_category ON household_staples(category);
CREATE INDEX IF NOT EXISTS idx_household_staples_frequency ON household_staples(frequency);

-- Indexes for staples_usage_history
CREATE INDEX IF NOT EXISTS idx_staples_usage_history_staple_id ON staples_usage_history(household_staple_id);
CREATE INDEX IF NOT EXISTS idx_staples_usage_history_meal_plan ON staples_usage_history(meal_plan_id);
CREATE INDEX IF NOT EXISTS idx_staples_usage_history_date ON staples_usage_history(added_to_list_at);

-- Composite index for meal plan staples queries
CREATE INDEX IF NOT EXISTS idx_meal_plans_staples ON meal_plans(owner_id, staples_included);
...
EXCEPTION
    WHEN undefined_object THEN
        -- Trigger doesn't exist, which is fine
        NULL;
END $$;
...
-- First, safely drop the existing trigger
DO $$
BEGIN
...
EXCEPTION
    WHEN undefined_object THEN
        -- Triggers don't exist, which is fine
        NULL;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '=== Migration 004 V2 Completed Successfully ===';
